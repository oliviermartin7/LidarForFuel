...
computeLiDARMetrics=function(PADval=NULL, # PAD values in m2/m3; either PAD or CBD should be provided
                            ...
                            asTibble=F) { # two different output formats depending on the need (plots or raster)
  
  zlow = 1.0 # limitation of lower uncertain strata in m, for TFL2 computation, etc
  ztmp = dz*(0.5+c(1:3)); zval_1mplus = min(ztmp[ztmp>zlow]) # center height of first cell above zlow 
  ...
  CBHlow=NA;CBH=NA;CBH2=NA;
  CanBDmax=NA;CanBDmin=NA;CanBDlowmax=NA;CanBDlowmin=NA;CBD_cbhlow=NA;CBD_cbh=NA;CBD_cbh2=NA;
  ...
  if (is.na(CDH)) {CDH = CTH} #(nb missing data for CDH consist in 0.75m strata...)} 
  ...
  if (!is.na(CBHlow)) {if (CBHlow <= zval_1mplus+0.001) {CBHlow = NA_real_}} # on remet à NA les CBHlow correspondant à maille 1.25, car pas intéressant
  ...
  names(ladders) <- as.vector(rbind(paste0("LadderBot_", suf),paste0("LadderTop_", suf),paste0("LadderLoad_", suf),
...
#  }
  ...
  out=c(TFL2 = TFL2,FL_k1 = FL_k1,FL_k2 = FL_k2,
        ...
        LFL=LFL,CFL= CFL)#SFLeq0 = SFLeq0, SFLeq_int = SFLeq_intLFLlow=LFLlow, )
  if (asTibble) {
...
computeLiDARMetricsRast=function(PAD_rast=NULL, # PAD values in m2/m3; either PAD or CBD should be provided
