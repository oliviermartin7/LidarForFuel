...
subs_cat=catalog_select(catalog_Uchaux)
crs(subs_cat)="epsg:2154"
...
operation = function(raster,res=3){


# Perform dilation
dilated_raster <- terra::focal(raster, w = 9, fun = max)

# Perform erosion on the dilated raster (Closing = Dilation + Erosion)
closed_raster <- focal(dilated_raster, w = 9,fun=min)



# Perform erosion on the closed raster
eroded_raster <-  focal(closed_raster, w = 9, fun=min)


# Perform dilation on the eroded raster (Opening = Erosion + Dilation)
opened_raster <-  focal(eroded_raster, w = 9, fun = max)

# Plot the closed raster
return(opened_raster)
}


test=operation(rasterized )
fgetpolybushandtree=function(chunk){
  # read chunk
...
chm_bush <- lidR::rasterize_canopy(clip_zone_bush, res = 0.3, p2r()) # crée un model de surface à partir du ndp bush
...
chm_trees <- lidR::rasterize_canopy(clip_zone_trees, res = 0.3, p2r())
...
poly_trees[which(as.numeric(poly_trees$area)<0.1),]
...
mapview::mapview(shp_Route_de_Mornas[shp_Route_de_Mornas$type=="bush",2],legend=T,layer.name="Bush arbre (surface m²)")

mapview::mapview(shp_Route_de_Mornas,legend=T,layer.name="Bush arbre (surface m²)")
...
opt_progress(subs_cat)=T
opt_stop_early(subs_cat)=T

# 500m
opt_chunk_size(subs_cat)=0

# Do not use a Buffer, no need and slows down the process
opt_chunk_buffer(subs_cat) <- 0

# opt_laz_compression(catalog_Uchaux) = T

# Where to output the rasters
opt_output_files(subs_cat) <- "D:/LiDARandField_Data_Fuel/Data_LiDAR_IGN/OLD/Uchaux/Cluster_arbustif_arbre_souscatalogUchaux/{ID}" # chemin vers le dossier de sortie des nouveau quadras

catalog_apply(subs_cat,fgetpolybushandtree)



library(sf)
library(tidyverse)
file_list <- list.files("D:/LiDARandField_Data_Fuel/Data_LiDAR_IGN/OLD/Uchaux/Cluster_arbustif_arbre_souscatalogUchaux/", pattern = "*shp", full.names = TRUE)
